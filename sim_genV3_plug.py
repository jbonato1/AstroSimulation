...
import pickle
from computeMI import computeMI_par_single_plug
    ...
    for alpha in [1,0.5]:
        for trial_id in range(len(trial_num)):
            ...
            for simulation in range(sim):
                ...
                ########### compute MI
                dict_out = computeMI_par_single_plug(data,spaceS,Sbins,RR)
                if simulation==0:
                    dict_gen = dict_out
                else:
                    for key in dict_out:
            ...
            file2 = open('/home/jbonato/Documents/AstroEncInfo/Results_R_all/MI_trial'+str(trial)+'_R'+str(RR)+'_space12_alpha'+str(alpha)+'.pkl','wb')
